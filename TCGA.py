...
# Same as UMAP_cancer_plot_1 however this uses umap plotting.
def UMAP_cancer_plot_3(expression_data, cancer_data, cancer_type, gene_mutation_data, gene_name, n_neighbors = 15, min_dist = 0.1, n_components = 2):
    """This function first filters for cancer type then trains UMAP only on that cancer type expression data. Using UMAP plotting"""
    RANDOM_STATE = 40
    single_cancer = cancer_data[cancer_data['cancer_type'] == cancer_type] # filter for the cancer type of interest
    ...
    stand_df = pd.DataFrame(data = standardized_data,
                            index = expr_data_cancer.index,
                            columns = expr_data_cancer.columns)
    
    mut_df = stand_df.merge(gene_mutation_data[gene_name], left_index = True, right_index = True)
    
    reducer = umap.UMAP(n_neighbors = n_neighbors,
                        min_dist = min_dist,
                        random_state = RANDOM_STATE,
                        n_components = n_components ) #initializing UMAP model
    
    umap_data = reducer.fit(mut_df.loc[:, mut_df.columns != gene_name + '_y']) # Fitting UMAP model to expression data and putting back into dataframe.
    
    
    umap.plot.points(umap_data, labels = mut_df[gene_name + '_y'], theme = 'fire')
    
    plt.title('UMAP Plot of LGG using Gene: ' + gene_name)
    plt.ylabel()
